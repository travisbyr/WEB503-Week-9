...
function createListElemnet() {
  ...
  li.addEventListener("click", function () {
    ...
    if (finished) {
      // If finished
      removeButton.appendChild(document.createTextNode("remove")); // Insert remove text to the button
      removeButton.classList = "deleteButton"; // Add class
      li.appendChild(removeButton); // Add class to li element
      removeButton.addEventListener("click", function () {
        // add event to remove button variable
...
function addListAfterPress(event) {
  ...
  if (inputLength() > 0 && event.keyCode == 13) {
    // If Input lengeht is greater 0 and event key code equals 13
    createListElement(); // Create a item list
  }
}

button.addEvenetListener("click", addListAfterClick);   // Create button

input.addEventListener("keypress", addListAfterPress);  // Create input